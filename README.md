...
- **拓扑方案编辑器** — 画布拖拽搭建电气拓扑图，50+ 标准元件、13 种线缆类型、连接点自动吸附
- **管理员后台** — 用户管理、产品编码 CRUD、需求模块配置、大类管理、项目管理、拓扑方案管理、元件库管理
...
三栏布局：**元件库面板（170px，可折叠） | 画布（可缩放） | 属性面板（180px）**
...
| 线缆类型 | 13 种线缆（网线/485/电源/交流/直流/高压/CAN/STS 等），颜色+标签区分 |
| 线缆图例 | 浮动面板显示线缆类型图例，鼠标拖拽自由移动 |
| 属性面板 | 选中元件编辑标签/X/Y，支持复制/删除 |
...
| 缩放范围 | 30%~200%，超出范围自动限制 |
| 重置视图 | 一键恢复原始大小和位置 |
| CT 互感器吸附 | 拖放至线缆附近自动吸附到中点，参数 t(0-1) 调节位置 |
| 全屏 | 支持全屏编辑模式，自动适配画布尺寸 |
...
| 子标签 | 功能 |
|--------|------|
| 元件管理 | 50+ 元件 CRUD，按分类筛选，搜索，SVG 预览，连接点配置，导入/导出 JSON |
